...
class WBCEWithLogitLoss(nn.Module):
    ...
    def __init__(self, weight=1.0, ignore_index=None, reduction='mean', eps=1e-6, smooth=0.01):
    ...
    def forward(self, output, target):
        ...
        if self.ignore_index is not None:
            ...
            # if ignore_index=1, only FP; if ignore_index=0, output=TP, target =P  than TP+N
        ...
        # avoid `nan` loss
        output = torch.clamp(output, min=self.eps, max=1.0 - self.eps)
        # soft label
...
class MutiClassCrossEntropyLoss(nn.Module):
    ...
    '''
    def __init__(self, class_weight=None, ignore_index=None, normalization='sigmoid', reduction='mean',
                 eps=1e-6, smooth=0.01):
    ...
    def forward(self, output, target, weights=None):
        ...
        # soft label
        target = torch.clamp(target.float(), min=self.smooth, max=1.0 - self.smooth)

        log_p = torch.log(output)

        mask = torch.ones_like(log_p, dtype=torch.float32)
        if self.class_weight is not None:
        ...
        if weights is not None:
            ...
            weights = weights.expand_as(log_p)

        loss = torch.sum(-target * log_p * mask * weights, dim=1)   # N, *
